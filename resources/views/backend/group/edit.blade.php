...
@section('content')
    ...
    <div class="card card-default mt-2">
        ...
        <div class="card-body">
            <form method="post" action="{{route('group.update',$group->name)}}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="name" placeholder="Enter The Unique Group Name" value="{{$group->name}}">
                            @error('name')
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="display_name" placeholder="Enter The Group Display Name" value="{{$group->display_name}}">
                            @error('display_name')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                                                        ...
                                                        @foreach($permissions as $permission)
                                                            @if($permission['name'] === strtolower($permissionName).'_'.strtolower($modelName))
                                                                <label class="control  control-checkbox checkbox-primary">{{ucfirst($permissionName).' '.$modelName}}
                                                                    <input type="checkbox" name="permissions[]" @if(in_array($permission['id'],$group->permissions->pluck('id')->toArray())) checked @endif value="{{$permission['id']}}">
