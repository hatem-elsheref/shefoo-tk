...
@section('content')
    <nav aria-label="breadcrumb" class="paths-nav">
        <ol class="breadcrumb breadcrumb-inverse">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{__('backend.edit_post')}}</li>
        </ol>
    </nav>
    <div class="card card-default mt-2">
        <div class="card-header card-header-border-bottom justify-content-between">
            <h2>{{__('backend.edit_post')}}</h2>
            <a href="{{route('Post.index')}}" title="{{__('backend.show_all_posts')}}" class="btn btn-sm btn-primary"><i class="mdi mdi-backup-restore"></i></a>
        </div>
        <div class="card-body">
            <form method="post" id="d" action="{{route('Post.update',$post->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" id="title" class="form-control" name="title" placeholder="{{__('backend.enter')}} {{__('backend.post_title')}}" value="{{$post->title}}">
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" class="form-control">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" @if($post->category_id == $category->id) selected @endif>{{ ucwords($category->name) }}</option>
                                @endforeach
                            </select>
                            @error('category')
                            <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                ...
                                <option value="drafted" @if($post->status == 'drafted') selected @endif>Drafted</option>
                            </select>
                            @error('status')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <select class="post-meta-tags form-control" name="meta[]" multiple="multiple">
                                @foreach((array) unserialize($post->meta) as $metaTag)
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            @enderror
                            <img style="width: 60px;height:60px" id="img-preview" class="mt-2 img-responsive  img-fluid" src="{{ uploads($post->image) }}">
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <select class="post-tags form-control" name="tags[]" multiple="multiple">
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}" @if(in_array($tag->id,$post->postTags())) selected @endif>{{ ucwords($tag->name) }}</option>
                                @endforeach
                            </select>
                            @error('tags')
                            <span class="text-danger">{{$message}}</span><br>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <textarea name="content" id="editor" class="form-control" style="height: 500px">{!! $post->content !!}</textarea>
                            @error('content')
                            <span class="text-danger">{{$message}}</span><br>
                            @enderror
                        </div>
